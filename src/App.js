...
const TagChip=({tag,onRemove})=>(<span style={{display:"inline-flex",alignItems:"center",gap:4,padding:"2px 8px",borderRadius:20,background:tag.color+"22",color:tag.color,fontSize:11,fontWeight:600,border:`1px solid ${tag.color}44`}}>{tag.name}{onRemove&&<span onClick={onRemove} style={{cursor:"pointer",fontSize:10,opacity:.7}}>✕</span>}</span>);
const Checkbox=({checked,onChange,size=18})=>(<div onClick={onChange} style={{width:size,height:size,borderRadius:5,border:`2px solid ${checked?COLORS.accent:COLORS.border}`,background:checked?COLORS.accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,transition:"all .15s"}}>{checked&&<span style={{color:"#fff",fontSize:size*.6,fontWeight:700}}>✓</span>}</div>);
...
const TaskForm=({task,tags,onSave,onClose,isChild,defaultDate,defaultTime})=>{
  ...
  const parentTags=activeTags.filter(t=>!t.parentId);
  const childTags=pid=>activeTags.filter(t=>t.parentId===pid);
  return (
      ...
      <div style={{marginBottom:14}}>
        ...
        {parentTags.map(pt=>(
          <div key={pt.id} style={{marginBottom:8}}>
            ...
            {childTags(pt.id).length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:6,paddingLeft:16}}>{childTags(pt.id).map(ct=><div key={ct.id} onClick={()=>tog(ct.id)} style={{padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:600,cursor:"pointer",border:`1px solid ${ct.color}44`,background:f.tags.includes(ct.id)?ct.color+"33":"transparent",color:f.tags.includes(ct.id)?ct.color:COLORS.textMuted}}>└ {ct.name}</div>)}</div>}
          </div>
        ))}
      </div>
...
const TaskRow=({task,tags,depth=0,onEdit,onDelete,onToggle,onAddChild,onDragStart})=>{
  const [exp,setExp]=useState(true);
  const tTags=tags.filter(t=>task.tags?.includes(t.id)&&!t.archived);
  ...
  return (
    <div style={{marginLeft:depth*22}}>
      ...
      {exp&&task.children?.map(c=><TaskRow key={c.id} task={c} tags={tags} depth={depth+1} onEdit={onEdit} onDelete={onDelete} onToggle={onToggle} onAddChild={onAddChild} onDragStart={onDragStart}/>)}
    </div>
  );
};

const ListView=({tasks,tags,filters,onEdit,onDelete,onToggle,onAddChild,onDragStart})=>{
  ...
  const Sec=({title,items,accent,hint})=>items.length===0?null:(<div style={{marginBottom:28}}><div style={{fontSize:12,fontWeight:700,color:accent,textTransform:"uppercase",letterSpacing:1,marginBottom:4,display:"flex",alignItems:"center",gap:8}}><div style={{width:20,height:2,background:accent}}></div>{title} ({items.length})</div>{hint&&<div style={{fontSize:11,color:COLORS.textMuted,marginBottom:10,paddingLeft:28}}>💡 {hint}</div>}{items.map(t=><TaskRow key={t.id} task={t} tags={tags} onEdit={onEdit} onDelete={onDelete} onToggle={onToggle} onAddChild={onAddChild} onDragStart={onDragStart}/>)}</div>);
  return (<div><Sec title="習慣・繰り返し" items={habits} accent={COLORS.success}/><Sec title="タスク" items={regular} accent={COLORS.accent}/><Sec title="あとでやる" items={later} accent={COLORS.warning} hint="PCでは日・週・月ビューにドラッグして日時設定できます"/>{filtered.length===0&&<div style={{textAlign:"center",padding:"60px 0",color:COLORS.textMuted}}><div style={{fontSize:48,marginBottom:12}}>🎉</div><div>タスクがありません</div></div>}</div>);
};

const DayView=({tasks,tags,today,onUpdateTask,onAddTask,dragTask})=>{
  const [dropHour,setDropHour]=useState(null);
  const allTasks=flattenTasks(tasks);
  const todayTasks=allTasks.filter(t=>{if(t.repeat==="毎日")return true;if(t.repeat==="平日のみ"){const d=new Date(today).getDay();return d>=1&&d<=5;}return isSameDay(t.startDate,today)||isSameDay(t.deadlineDate,today);});
  const timed=todayTasks.filter(t=>t.startTime);
  const untimed=todayTasks.filter(t=>!t.startTime);
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 280px",gap:24}}>
      <div>
        <div style={{fontSize:12,fontWeight:700,color:COLORS.textMuted,marginBottom:12,textTransform:"uppercase",letterSpacing:1}}>タイムライン {dragTask&&<span style={{color:COLORS.accent,fontWeight:400}}>← ここにドロップして日時設定</span>}</div>
        {HOURS.slice(6,23).map((hour,i)=>{
          const h=6+i,isDrop=dropHour===h;
          const ht=timed.filter(t=>t.startTime?.slice(0,2)===String(h).padStart(2,"0"));
          return (
            <div key={hour} onDragOver={e=>{e.preventDefault();setDropHour(h);}} onDragLeave={()=>setDropHour(null)} onDrop={e=>{e.preventDefault();setDropHour(null);if(dragTask)onUpdateTask({...dragTask,startDate:today,startTime:`${String(h).padStart(2,"0")}:00`,isLater:false});}} style={{display:"grid",gridTemplateColumns:"50px 1fr",minHeight:60,borderTop:`1px solid ${COLORS.border}`,background:isDrop?"rgba(108,99,255,0.1)":"transparent",transition:"background .15s"}}>
              ...
              <div style={{padding:"4px 0 4px 12px",position:"relative"}}>
                {isDrop&&dragTask?<div style={{position:"absolute",inset:"2px 4px",border:`2px dashed ${COLORS.accent}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:COLORS.accent,pointerEvents:"none"}}>"{dragTask.title}" を {hour} に設定</div>:
                ht.length>0?ht.map(t=>{const c=tags.find(tg=>t.tags?.includes(tg.id))?.color||COLORS.accent;return(<div key={t.id} style={{background:c+"22",borderLeft:`4px solid ${c}`,borderRadius:"0 10px 10px 0",padding:"8px 12px",marginBottom:4}}><div style={{fontSize:13,fontWeight:700,color:c}}>{t.title}</div><div style={{fontSize:11,color:COLORS.textMuted}}>{t.startTime}{t.endTime?` 〜 ${t.endTime}`:""}{t.deadlineDate?` | ⚠ ${formatDateTime(t.deadlineDate,t.deadlineTime)}`:""}</div></div>);}):
                <div onClick={()=>onAddTask(today,h)} style={{height:52,cursor:"pointer",borderRadius:6,display:"flex",alignItems:"center",paddingLeft:8,fontSize:11,color:"transparent",transition:"all .15s"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(108,99,255,0.08)";e.currentTarget.style.color=COLORS.textMuted;}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color="transparent";}}>+ タスクを追加</div>}
              </div>
            </div>
          );
        })}
      ...
      <div>
        ...
        {untimed.length===0?<div style={{color:COLORS.textMuted,fontSize:13}}>なし</div>:untimed.map(t=>{const c=tags.find(tg=>t.tags?.includes(tg.id))?.color||COLORS.accent;return <div key={t.id} style={{background:COLORS.surface,borderRadius:10,padding:"10px 14px",marginBottom:8,borderLeft:`3px solid ${c}`}}><div style={{fontSize:13,fontWeight:600}}>{t.title}</div>{t.deadlineDate&&<div style={{fontSize:11,color:COLORS.warning,marginTop:2}}>⚠ {formatDateTime(t.deadlineDate,t.deadlineTime)}</div>}</div>;})}
      </div>
    </div>
  );
};

const WeekView=({tasks,tags,today,onUpdateTask,onAddTask,dragTask})=>{
  const weekDates=getWeekDates(today);
  const [dropCell,setDropCell]=useState(null);
  const allTasks=flattenTasks(tasks);
  const getDay=date=>allTasks.filter(t=>{if(t.repeat==="毎日")return true;if(t.repeat==="平日のみ"){const d=new Date(date).getDay();return d>=1&&d<=5;}if(t.repeat==="毎週"&&t.startDate)return new Date(t.startDate).getDay()===new Date(date).getDay();return isSameDay(t.startDate,date)||isSameDay(t.deadlineDate,date);});
  const cellKey=(d,h)=>`${d}_${h}`;
  return (
    <div style={{overflowX:"auto"}}>
      {dragTask&&<div style={{fontSize:12,color:COLORS.accent,marginBottom:8,padding:"6px 12px",background:COLORS.accentSoft,borderRadius:8}}>📌 "{dragTask.title}" をカレンダーにドロップして日時設定</div>}
        ...
        {HOURS.slice(6,23).map((hour,i)=>{
          ...
          return [
            ...
            ...weekDates.map(d=>{
              ...
              const key=cellKey(d,h),isDrop=dropCell===key;
              return(<div key={d+hour} onDragOver={e=>{e.preventDefault();setDropCell(key);}} onDragLeave={()=>setDropCell(null)} onDrop={e=>{e.preventDefault();setDropCell(null);if(dragTask)onUpdateTask({...dragTask,startDate:d,startTime:`${String(h).padStart(2,"0")}:00`,isLater:false});}} onClick={()=>{if(!dragTask)onAddTask(d,h);}} style={{borderTop:`1px solid ${COLORS.border}22`,height:52,padding:2,background:isDrop?"rgba(108,99,255,0.15)":"transparent",transition:"background .15s",cursor:"pointer"}}>
                {isDrop&&dragTask?<div style={{height:"100%",border:`2px dashed ${COLORS.accent}`,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:COLORS.accent}}>{hour}</div>:
                dts.length>0?dts.map(t=>{const c=tags.find(tg=>t.tags?.includes(tg.id))?.color||COLORS.accent;return <div key={t.id} style={{background:c+"33",borderLeft:`3px solid ${c}`,borderRadius:"0 6px 6px 0",padding:"2px 6px",fontSize:10,color:c,fontWeight:600,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",marginBottom:2}}>{t.startTime} {t.title}</div>;}):
                <div style={{height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:"transparent",transition:"color .15s"}} onMouseEnter={e=>e.currentTarget.style.color=COLORS.textMuted} onMouseLeave={e=>e.currentTarget.style.color="transparent"}>+</div>}
              </div>);
            })
          ];
        })}
...
const MonthView=({tasks,tags,today,onUpdateTask,onAddTask,dragTask})=>{
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}><Btn onClick={()=>{if(vm===0){setVy(y=>y-1);setVm(11);}else setVm(m=>m-1);}}>‹ 前月</Btn><span style={{fontWeight:700,fontSize:16}}>{vy}年 {MN[vm]}</span><Btn onClick={()=>{if(vm===11){setVy(y=>y+1);setVm(0);}else setVm(m=>m+1);}}>次月 ›</Btn></div>
      {dragTask&&<div style={{fontSize:12,color:COLORS.accent,marginBottom:8,padding:"6px 12px",background:COLORS.accentSoft,borderRadius:8}}>📌 "{dragTask.title}" を日付にドロップして開始日設定</div>}
      <div style={{overflowX:"auto"}}>
        <div style={{minWidth:dim*30+180}}>
          ...
          {allTasks.map(task=>{const bar=getBar(task),c=tags.find(t=>task.tags?.includes(t.id))?.color||COLORS.accent;return <div key={task.id} style={{display:"flex",alignItems:"center",marginBottom:6,height:28}}><div style={{width:180,flexShrink:0,fontSize:12,paddingRight:8,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",color:COLORS.textSoft}}>{task.title}</div><div style={{position:"relative",height:"100%",flex:1}}>{bar&&<div style={{position:"absolute",left:(bar.startDay-1)*30,width:bar.width*30-4,height:22,top:3,background:c+"33",border:`1px solid ${c}55`,borderRadius:6,display:"flex",alignItems:"center",paddingLeft:8,fontSize:10,color:c,fontWeight:600,overflow:"hidden",whiteSpace:"nowrap"}}>{bar.width>2?task.title.slice(0,15):""}</div>}</div></div>;})}
        </div>
      </div>
    </div>
  );
...
const TagsView=({tags,setTags})=>{
  ...
  const EditBox=({id})=>editId===id&&editForm?(<div style={{background:COLORS.bg,borderRadius:10,padding:12,marginTop:8,display:"flex",gap:10,alignItems:"flex-end"}}><div style={{flex:1}}><Inp label="タグ名" value={editForm.name} onChange={v=>setEditForm(f=>({...f,name:v}))}/></div><div style={{marginBottom:14}}><div style={{fontSize:12,color:COLORS.textMuted,marginBottom:5,fontWeight:600}}>色</div><input type="color" value={editForm.color} onChange={e=>setEditForm(f=>({...f,color:e.target.value}))} style={{width:44,height:38,borderRadius:8,border:`1px solid ${COLORS.border}`,background:"none",cursor:"pointer",padding:2}}/></div><div style={{marginBottom:14,display:"flex",gap:6}}><Btn variant="accent" onClick={()=>saveEdit(id)}>保存</Btn><Btn onClick={()=>setEditId(null)}>✕</Btn></div></div>):null;
  return (
    <div>
        ...
        {parentTags.map(pt=>(
          <div key={pt.id} style={{background:COLORS.surface,borderRadius:12,padding:16,border:`1px solid ${pt.color}44`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:12,height:12,borderRadius:"50%",background:pt.color}}></div><span style={{fontWeight:700,color:pt.color,fontSize:14}}>{pt.name}</span><span style={{fontSize:11,color:COLORS.textMuted}}>親タグ</span></div>
              <div style={{display:"flex",gap:6}}><Btn onClick={()=>startEdit(pt)} style={{padding:"3px 10px",fontSize:11}}>編集</Btn><Btn variant="danger" onClick={()=>archive(pt.id)} style={{padding:"3px 10px",fontSize:11}}>アーカイブ</Btn></div>
            </div>
            <EditBox id={pt.id}/>
            {childTags(pt.id).length>0&&(
              <div style={{paddingLeft:22,display:"flex",flexDirection:"column",gap:6,marginTop:12}}>
                {childTags(pt.id).map(ct=>(
                  <div key={ct.id} style={{background:COLORS.bg,borderRadius:8,border:`1px solid ${ct.color}33`}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:ct.color}}></div><span style={{fontSize:13,color:ct.color,fontWeight:600}}>{ct.name}</span><span style={{fontSize:10,color:COLORS.textMuted}}>小タグ</span></div>
                      <div style={{display:"flex",gap:6}}><Btn onClick={()=>startEdit(ct)} style={{padding:"2px 8px",fontSize:11}}>編集</Btn><Btn variant="danger" onClick={()=>archive(ct.id)} style={{padding:"2px 8px",fontSize:11}}>アーカイブ</Btn></div>
                    </div>
                    <div style={{padding:"0 10px"}}><EditBox id={ct.id}/></div>
                  </div>
                ))}
              </div>
            )}
      ...
      {archivedTags.length>0&&(
        <div style={{marginTop:24}}>
          <button onClick={()=>setShowArchived(!showArchived)} style={{background:"none",border:"none",color:COLORS.textMuted,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6,marginBottom:12}}>{showArchived?"▼":"▶"} アーカイブ済み ({archivedTags.length})</button>
...
export default function App() {
  ...
  const handleDragStart=(e,task)=>{e.dataTransfer.effectAllowed="move";setDragTask(task);};
  ...
  return (
    <>
      <style>{G}</style>
      <div style={{minHeight:"100vh",background:COLORS.bg,display:"flex"}} onDragEnd={()=>setDragTask(null)}>
        ...
        <div style={{marginLeft:216,flex:1,padding:32,maxWidth:"calc(100% - 216px)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}}>
            ...
            {["list","day","week"].includes(view)&&<Btn variant="accent" onClick={()=>{setDefaultDate(null);setDefaultTime(null);setEditTask(null);setAddChildTo(null);setShowForm(true);}}>+ タスクを追加</Btn>}
          </div>
          {view==="list"&&<ListView tasks={tasks} tags={tags} filters={filters} onEdit={t=>{setEditTask(t);setShowForm(true);}} onDelete={id=>updT(delTree(tasks,id))} onToggle={id=>updT(updTree(tasks,id,t=>({...t,done:!t.done})))} onAddChild={pid=>{setAddChildTo(pid);setShowForm(true);}} onDragStart={handleDragStart}/>}
          {view==="day"&&<DayView tasks={tasks} tags={tags} today={today} onUpdateTask={handleUpdateTask} onAddTask={handleAddTask} dragTask={dragTask}/>}
          {view==="week"&&<WeekView tasks={tasks} tags={tags} today={today} onUpdateTask={handleUpdateTask} onAddTask={handleAddTask} dragTask={dragTask}/>}
          {view==="month"&&<MonthView tasks={tasks} tags={tags} today={today} onUpdateTask={handleUpdateTask} onAddTask={handleAddTask} dragTask={dragTask}/>}
