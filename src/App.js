...
const LaterPanel=({tasks,tags,dragTask,setDragTask})=>{
  ...
  return (
    <div style={{width:200,flexShrink:0,background:COLORS.surface,borderLeft:`1px solid ${COLORS.border}`,padding:14,overflowY:"auto"}}>
      <div style={{fontSize:11,fontWeight:700,color:COLORS.warning,marginBottom:10,textTransform:"uppercase",letterSpacing:1}}>📌 あとでやる</div>
      <div style={{fontSize:10,color:COLORS.textMuted,marginBottom:10}}>カレンダーにドラッグして日時設定</div>
      ...
      {laterTasks.map(t=>{
        ...
        return (
          <div key={t.id} draggable onDragStart={e=>{e.dataTransfer.effectAllowed="move";setDragTask(t);}} onDragEnd={()=>setDragTask(null)}
            ...
            <div style={{fontSize:12,fontWeight:600,color:COLORS.text,marginBottom:3}}>{t.title}</div>
            {t.deadlineDate&&<div style={{fontSize:10,color:COLORS.warning}}>⚠ {formatDate(t.deadlineDate)}</div>}
            <div style={{display:"flex",flexWrap:"wrap",gap:3,marginTop:3}}>
              {tags.filter(tg=>t.tags?.includes(tg.id)).map(tg=><span key={tg.id} style={{fontSize:9,color:tg.color,background:tg.color+"22",padding:"1px 5px",borderRadius:10}}>{tg.name}</span>)}
            </div>
          </div>
        );
      })}
            ...
            ...weekDates.map(d=>{
              ...
              return (
                <div key={d+hour} onDragOver={e=>{e.preventDefault();setDropCell(key);}} onDragLeave={()=>setDropCell(null)} onDrop={e=>{e.preventDefault();setDropCell(null);if(dragTask)onUpdateTask({...dragTask,startDate:d,startTime:`${String(h).padStart(2,"0")}:00`,isLater:false});}}
                  ...
                  {isDrop&&dragTask?<div style={{height:"100%",border:`2px dashed ${COLORS.accent}`,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:COLORS.accent}}>{hour}</div>:dts.map(t=>{const c=tags.find(tg=>t.tags?.includes(tg.id))?.color||COLORS.accent;return <div key={t.id} style={{background:c+"33",borderLeft:`3px solid ${c}`,borderRadius:"0 6px 6px 0",padding:"2px 6px",fontSize:10,color:c,fontWeight:600,overflow:"hidden",whiteSpace:"nowrap",textOverflow:"ellipsis",marginBottom:2}}>{t.startTime} {t.title}</div>;})}
                </div>
              );
            })
          ];
...
const TagsView=({tags,setTags})=>{
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {parentTags.map(pt=>(
          <div key={pt.id} style={{background:COLORS.surface,borderRadius:12,padding:16,border:`1px solid ${pt.color}44`}}>
            ...
            <EditRow t={pt}/>
            {childTags(pt.id).length>0&&<div style={{paddingLeft:22,marginTop:12,display:"flex",flexDirection:"column",gap:6}}>
              {childTags(pt.id).map(ct=>(<div key={ct.id} style={{background:COLORS.bg,borderRadius:8,border:`1px solid ${ct.color}33`,padding:"6px 10px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:ct.color}}></div><span style={{fontSize:13,color:ct.color,fontWeight:600}}>{ct.name}</span><span style={{fontSize:10,color:COLORS.textMuted}}>小タグ</span></div><div style={{display:"flex",gap:6}}><Btn onClick={()=>startEdit(ct)} style={{padding:"2px 8px",fontSize:11}}>編集</Btn><Btn variant="danger" onClick={()=>archive(ct.id)} style={{padding:"2px 8px",fontSize:11}}>アーカイブ</Btn></div></div>
                <EditRow t={ct}/>
              </div>))}
            </div>}
          </div>
      ...
      {archivedTags.length>0&&<div style={{marginTop:24}}>
