...
const G=`
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .task-chip:hover{filter:brightness(1.2);transform:scale(1.02);}
...
// ---- タスクポップアップ（各ビューで使う） ----
const TaskPopup=({task,tags,onClose,onEdit,onToggle,onDelete,anchor})=>{
  ...
  return (
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:500}}>
      <div onClick={e=>e.stopPropagation()} style={{position:"fixed",top:anchor?.y||100,left:anchor?.x||100,background:COLORS.surface,borderRadius:14,padding:16,border:`1px solid ${COLORS.border}`,width:280,boxShadow:"0 8px 32px rgba(0,0,0,0.5)",zIndex:501}}>
        <div style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:12}}>
          ...
          <div style={{flex:1}}>
            ...
            {tTags.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:4,marginTop:6}}>{tTags.map(t=><span key={t.id} style={{fontSize:10,color:t.color,background:t.color+"22",padding:"1px 6px",borderRadius:10,fontWeight:600}}>{t.name}</span>)}</div>}
          </div>
        </div>
        <div style={{fontSize:12,color:COLORS.textMuted,display:"flex",flexDirection:"column",gap:4,marginBottom:12}}>
          ...
          {task.memo&&<div style={{color:COLORS.textSoft,fontStyle:"italic",marginTop:4}}>{task.memo}</div>}
        </div>
...
const LaterPanel=({tasks,tags,dragTask,setDragTask})=>{
  ...
  return (
    <div style={{width:200,flexShrink:0,background:COLORS.surface,borderLeft:`1px solid ${COLORS.border}`,padding:14,overflowY:"auto"}}>
      <div style={{fontSize:11,fontWeight:700,color:COLORS.warning,marginBottom:6,textTransform:"uppercase",letterSpacing:1}}>📌 あとでやる</div>
      <div style={{fontSize:10,color:COLORS.textMuted,marginBottom:10}}>ドラッグして日時設定</div>
      {laterTasks.length===0&&<div style={{fontSize:12,color:COLORS.textMuted,textAlign:"center",padding:"20px 0"}}>なし</div>}
      {laterTasks.map(t=>{
        const c=tags.find(tg=>t.tags?.includes(tg.id))?.color||COLORS.accent;
        const isDragging=dragTask?.id===t.id;
        return (
          <div key={t.id} draggable onDragStart={e=>{e.dataTransfer.effectAllowed="move";setDragTask(t);}} onDragEnd={()=>setDragTask(null)}
            style={{background:isDragging?COLORS.accentSoft:COLORS.bg,borderLeft:`3px solid ${c}`,borderRadius:"0 8px 8px 0",padding:"8px 10px",marginBottom:8,cursor:"grab",opacity:isDragging?0.5:1,transition:"all .15s"}}>
...
const TaskForm=({task,tags,onSave,onClose,isChild,defaultDate,defaultTime})=>{
  const empty={id:"task_"+Date.now(),title:"",done:false,tags:[],memo:"",startDate:defaultDate||"",startTime:defaultTime||"",endDate:"",endTime:"",deadlineDate:"",deadlineTime:"",repeat:"なし",children:[],isLater:false};
  ...
  return (
    <Modal title={task?"タスクを編集":isChild?"子タスクを追加":"タスクを追加"} onClose={onClose} wide>
      ...
      <div style={{marginBottom:14}}>
        ...
        {parentTags.map(pt=>(<div key={pt.id} style={{marginBottom:8}}><div onClick={()=>tog(pt.id)} style={{display:"inline-flex",padding:"4px 12px",borderRadius:20,fontSize:12,fontWeight:700,cursor:"pointer",border:`1px solid ${pt.color}44`,background:f.tags.includes(pt.id)?pt.color+"33":"transparent",color:f.tags.includes(pt.id)?pt.color:COLORS.textMuted,marginBottom:4}}>{pt.name}</div>{childTags(pt.id).length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:6,paddingLeft:16}}>{childTags(pt.id).map(ct=><div key={ct.id} onClick={()=>tog(ct.id)} style={{padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:600,cursor:"pointer",border:`1px solid ${ct.color}44`,background:f.tags.includes(ct.id)?ct.color+"33":"transparent",color:f.tags.includes(ct.id)?ct.color:COLORS.textMuted}}>└ {ct.name}</div>)}</div>}</div>))}
      </div>
      <div style={{background:COLORS.bg,borderRadius:10,padding:12,marginBottom:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><Inp label="開始日" value={f.startDate} onChange={v=>upd("startDate",v)} type="date"/><Inp label="開始時刻（任意）" value={f.startTime} onChange={v=>upd("startTime",v)} type="time"/></div>
        <div style={{fontSize:12,color:COLORS.textMuted,marginBottom:8,fontWeight:600}}>⏹ 終了日時</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><Inp label="終了日" value={f.endDate} onChange={v=>upd("endDate",v)} type="date"/><Inp label="終了時刻（任意）" value={f.endTime} onChange={v=>upd("endTime",v)} type="time"/></div>
        <div style={{fontSize:12,color:COLORS.warning,marginBottom:8,fontWeight:600}}>⚠️ 締切日時</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}><Inp label="締切日" value={f.deadlineDate} onChange={v=>upd("deadlineDate",v)} type="date"/><Inp label="締切時刻（任意）" value={f.deadlineTime} onChange={v=>upd("deadlineTime",v)} type="time"/></div>
      </div>
      <div style={{fontSize:11,color:COLORS.textMuted,marginBottom:14,padding:"8px 12px",background:COLORS.accentSoft,borderRadius:8}}>💡 開始日を設定しないと「あとでやる」に自動追加されます</div>
      <Sel label="繰り返し" value={f.repeat} onChange={v=>upd("repeat",v)} options={REPEAT_OPTIONS}/>
      <div style={{marginBottom:14}}><div style={{fontSize:12,color:COLORS.textMuted,marginBottom:5,fontWeight:600}}>メモ</div><textarea value={f.memo} onChange={e=>upd("memo",e.target.value)} placeholder="メモ..." rows={3} style={{width:"100%",background:COLORS.bg,color:COLORS.text,padding:"9px 12px",borderRadius:8,border:`1px solid ${COLORS.border}`,fontSize:14,resize:"vertical"}}/></div>
...
const ListView=({tasks,tags,filters,onEdit,onDelete,onToggle,onAddChild})=>{
  ...
  const Sec=({title,items,accent})=>items.length===0?null:(<div style={{marginBottom:28}}><div style={{fontSize:12,fontWeight:700,color:accent,textTransform:"uppercase",letterSpacing:1,marginBottom:12,display:"flex",alignItems:"center",gap:8}}><div style={{width:20,height:2,background:accent}}></div>{title} ({items.length})</div>{items.map(t=><TaskRow key={t.id} task={t} tags={tags} onEdit={onEdit} onDelete={onDelete} onToggle={onToggle} onAddChild={onAddChild}/>)}</div>);
...
// ---- タスクチップ（各ビューで使う小さなタスク表示） ----
const TaskChip=({task,tags,color,onPopup,compact})=>{
  const isOverdue=task.deadlineDate&&!task.done&&task.deadlineDate<new Date().toISOString().slice(0,10);
  return (
    <div className="task-chip" onClick={e=>{e.stopPropagation();onPopup(e,task);}}
      style={{background:task.done?COLORS.border+"44":color+"33",borderLeft:`3px solid ${task.done?COLORS.textMuted:color}`,borderRadius:"0 6px 6px 0",padding:compact?"2px 6px":"6px 10px",marginBottom:2,cursor:"pointer",transition:"all .15s",opacity:task.done?.6:1}}>
      <div style={{display:"flex",alignItems:"center",gap:4}}>
        <div style={{width:10,height:10,borderRadius:3,border:`2px solid ${task.done?COLORS.textMuted:color}`,background:task.done?color:"transparent",flexShrink:0}}/>
...
const DayView=({tasks,tags,today,onUpdateTask,onAddTask,onToggle,onEdit,onDelete,dragTask})=>{
  ...
  const handlePopup=(e,task)=>{const r=e.currentTarget.getBoundingClientRect();setPopup({task,x:Math.min(r.right+8,window.innerWidth-300),y:Math.min(r.top,window.innerHeight-300)});};
  return (
    <div style={{display:"grid",gridTemplateColumns:"1fr 220px",gap:16}}>
      <div>
        ...
        {HOURS.slice(6,23).map((hour,i)=>{
          ...
          return (
            <div key={hour} onDragOver={e=>{e.preventDefault();setDropHour(h);}} onDragLeave={()=>setDropHour(null)} onDrop={e=>{e.preventDefault();setDropHour(null);if(dragTask)onUpdateTask({...dragTask,startDate:today,startTime:`${String(h).padStart(2,"0")}:00`,isLater:false});}}
              style={{display:"grid",gridTemplateColumns:"50px 1fr",minHeight:56,borderTop:`1px solid ${COLORS.border}`,background:isDrop?"rgba(108,99,255,0.1)":"transparent",transition:"background .15s"}}>
              <div style={{fontSize:11,color:isDrop?COLORS.accent:COLORS.textMuted,paddingTop:4,paddingRight:8,textAlign:"right",fontWeight:isDrop?700:400}}>{hour}</div>
              <div style={{padding:"2px 0 2px 8px",position:"relative"}}>
                {isDrop&&dragTask?<div style={{position:"absolute",inset:"2px 4px",border:`2px dashed ${COLORS.accent}`,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:COLORS.accent,pointerEvents:"none",background:COLORS.accentSoft}}>"{dragTask.title}" を {hour} に設定</div>:
                ht.length>0?ht.map(t=>{const c=tags.find(tg=>t.tags?.includes(tg.id))?.color||COLORS.accent;return <TaskChip key={t.id} task={t} tags={tags} color={c} onPopup={handlePopup}/>;}):<div onClick={()=>onAddTask(today,h)} style={{height:48,cursor:"pointer",borderRadius:6,display:"flex",alignItems:"center",paddingLeft:8,fontSize:11,color:"transparent",transition:"all .15s"}} onMouseEnter={e=>{e.currentTarget.style.background="rgba(108,99,255,0.08)";e.currentTarget.style.color=COLORS.textMuted;}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color="transparent";}}>+ 追加</div>}
              </div>
            </div>
          );
        })}
        {untimed.length>0&&<div style={{marginTop:16}}><div style={{fontSize:12,fontWeight:700,color:COLORS.textMuted,marginBottom:8}}>時間未定</div>{untimed.map(t=>{const c=tags.find(tg=>t.tags?.includes(tg.id))?.color||COLORS.accent;return <TaskChip key={t.id} task={t} tags={tags} color={c} onPopup={handlePopup}/>;})}</div>}
      </div>
      <div style={{fontSize:10,color:COLORS.textMuted,textAlign:"center",paddingTop:40,opacity:dragTask?1:0,transition:"opacity .2s"}}>⬅ タイムラインにドロップ</div>
      {popup&&<TaskPopup task={popup.task} tags={tags} anchor={popup} onClose={()=>setPopup(null)} onEdit={onEdit} onToggle={t=>{onToggle(t);setPopup(null);}} onDelete={onDelete}/>}
    </div>
  );
};

const WeekView=({tasks,tags,today,onUpdateTask,onAddTask,onToggle,onEdit,onDelete,dragTask})=>{
  ...
  return (
    <div style={{overflowX:"auto"}}>
      <div style={{display:"grid",gridTemplateColumns:"56px repeat(7,1fr)",minWidth:700}}>
        <div></div>
        {weekDates.map((d,i)=>{const isT=d===today,dt=new Date(d);return(<div key={d} style={{padding:"8px 4px",textAlign:"center",borderBottom:`2px solid ${isT?COLORS.accent:COLORS.border}`,color:isT?COLORS.accent:COLORS.textSoft}}><div style={{fontSize:11,fontWeight:700}}>{DAYS_JP[i]}</div><div style={{fontSize:18,fontWeight:isT?700:400}}>{dt.getDate()}</div></div>);})}
        {HOURS.slice(6,23).map((hour,i)=>{
          const h=6+i;
          return [
            ...
            ...weekDates.map(d=>{
              ...
              return (
                <div key={d+hour} onDragOver={e=>{e.preventDefault();setDropCell(key);}} onDragLeave={()=>setDropCell(null)} onDrop={e=>{e.preventDefault();setDropCell(null);if(dragTask)onUpdateTask({...dragTask,startDate:d,startTime:`${String(h).padStart(2,"0")}:00`,isLater:false});}}
                  style={{borderTop:`1px solid ${COLORS.border}22`,height:52,padding:2,background:isDrop?"rgba(108,99,255,0.15)":"transparent",transition:"background .15s",cursor:"pointer"}} onClick={()=>{if(!dragTask)onAddTask(d,h);}}>
                  {isDrop&&dragTask?<div style={{height:"100%",border:`2px dashed ${COLORS.accent}`,borderRadius:6,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:COLORS.accent}}>{hour}</div>:dts.map(t=>{const c=tags.find(tg=>t.tags?.includes(tg.id))?.color||COLORS.accent;return <TaskChip key={t.id} task={t} tags={tags} color={c} onPopup={handlePopup} compact/>;})}</div>
              );
            })
          ];
        })}
...
const MonthView=({tasks,tags,today,onUpdateTask,onAddTask,onToggle,onEdit,onDelete,dragTask})=>{
  ...
  const allTasks=flattenTasks(tasks).filter(t=>(t.startDate||t.deadlineDate)&&!(t.isLater||isAutoLater(t)));
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:16}}><Btn onClick={()=>{if(vm===0){setVy(y=>y-1);setVm(11);}else setVm(m=>m-1);}}>‹ 前月</Btn><span style={{fontWeight:700,fontSize:16}}>{vy}年 {MN[vm]}</span><Btn onClick={()=>{if(vm===11){setVy(y=>y+1);setVm(0);}else setVm(m=>m+1);}}>次月 ›</Btn></div>
      <div style={{overflowX:"auto"}}>
        <div style={{minWidth:dim*30+180}}>
          <div style={{display:"flex",borderBottom:`1px solid ${COLORS.border}`,paddingBottom:4,marginBottom:4}}>
            ...
            {Array.from({length:dim},(_,i)=>{const d=i+1,isT=dateStr(d)===today,isDrop=dropDay===d;return <div key={d} onDragOver={e=>{e.preventDefault();setDropDay(d);}} onDragLeave={()=>setDropDay(null)} onDrop={e=>{e.preventDefault();setDropDay(null);if(dragTask)onUpdateTask({...dragTask,startDate:dateStr(d),isLater:false});}} onClick={()=>{if(!dragTask)onAddTask(dateStr(d),null);}} style={{width:30,flexShrink:0,textAlign:"center",fontSize:10,fontWeight:isT?700:400,color:isDrop?COLORS.accent:isT?COLORS.accent:COLORS.textMuted,background:isDrop?"rgba(108,99,255,0.2)":"transparent",borderRadius:4,cursor:"pointer",padding:"2px 0"}}>{d}</div>;})}
          </div>
          {allTasks.map(task=>{
            ...
            return <div key={task.id} style={{display:"flex",alignItems:"center",marginBottom:6,height:28}}>
              ...
              <div style={{position:"relative",height:"100%",flex:1}}>
                {bar&&<div onClick={e=>handlePopup(e,task)} style={{position:"absolute",left:(bar.startDay-1)*30,width:bar.width*30-4,height:22,top:3,background:task.done?COLORS.border+"44":c+"33",border:`1px solid ${task.done?COLORS.textMuted:c}55`,borderRadius:6,display:"flex",alignItems:"center",paddingLeft:8,fontSize:10,color:task.done?COLORS.textMuted:c,fontWeight:600,overflow:"hidden",whiteSpace:"nowrap",cursor:"pointer",textDecoration:task.done?"line-through":"none",transition:"all .15s"}} className="task-chip">{bar.width>2?task.title.slice(0,15):""}</div>}
              </div>
            </div>;
          })}
        </div>
...
const TagsView=({tags,setTags})=>{
  ...
  const EditRow=({t})=>editId===t.id&&editForm?(<div style={{background:COLORS.bg,borderRadius:10,padding:12,marginTop:8,display:"flex",gap:10,alignItems:"flex-end"}}><div style={{flex:1}}><Inp label="タグ名" value={editForm.name} onChange={v=>setEditForm(f=>({...f,name:v}))}/></div><div style={{marginBottom:14}}><div style={{fontSize:12,color:COLORS.textMuted,marginBottom:5,fontWeight:600}}>色</div><input type="color" value={editForm.color} onChange={e=>setEditForm(f=>({...f,color:e.target.value}))} style={{width:44,height:38,borderRadius:8,border:`1px solid ${COLORS.border}`,background:"none",cursor:"pointer",padding:2}}/></div><div style={{marginBottom:14,display:"flex",gap:6}}><Btn variant="accent" onClick={()=>saveEdit(t.id)}>保存</Btn><Btn onClick={()=>setEditId(null)}>✕</Btn></div></div>):null;
  return (
    <div>
      <div style={{background:COLORS.surface,borderRadius:14,padding:20,border:`1px solid ${COLORS.border}`,marginBottom:20}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 80px",gap:12,marginBottom:12}}><Inp label="タグ名" value={form.name} onChange={v=>setForm(f=>({...f,name:v}))} placeholder="タグ名..."/><div><div style={{fontSize:12,color:COLORS.textMuted,marginBottom:5,fontWeight:600}}>色</div><input type="color" value={form.color} onChange={e=>setForm(f=>({...f,color:e.target.value}))} style={{width:"100%",height:38,borderRadius:8,border:`1px solid ${COLORS.border}`,background:"none",cursor:"pointer",padding:2}}/></div></div>
        <div style={{marginBottom:12}}><div style={{fontSize:12,color:COLORS.textMuted,marginBottom:5,fontWeight:600}}>親タグ（小タグとして追加する場合）</div><select value={form.parentId||""} onChange={e=>setForm(f=>({...f,parentId:e.target.value||null}))} style={{width:"100%",background:COLORS.bg,color:COLORS.text,padding:"9px 12px",borderRadius:8,border:`1px solid ${COLORS.border}`,fontSize:14}}><option value="">なし（親タグとして作成）</option>{parentTags.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {parentTags.map(pt=>(
          <div key={pt.id} style={{background:COLORS.surface,borderRadius:12,padding:16,border:`1px solid ${pt.color}44`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:12,height:12,borderRadius:"50%",background:pt.color}}></div><span style={{fontWeight:700,color:pt.color,fontSize:14}}>{pt.name}</span><span style={{fontSize:11,color:COLORS.textMuted}}>親タグ</span></div><div style={{display:"flex",gap:6}}><Btn onClick={()=>startEdit(pt)} style={{padding:"3px 10px",fontSize:11}}>編集</Btn><Btn variant="danger" onClick={()=>archive(pt.id)} style={{padding:"3px 10px",fontSize:11}}>アーカイブ</Btn></div></div>
            <EditRow t={pt}/>
            {childTags(pt.id).length>0&&<div style={{paddingLeft:22,marginTop:12,display:"flex",flexDirection:"column",gap:6}}>{childTags(pt.id).map(ct=>(<div key={ct.id} style={{background:COLORS.bg,borderRadius:8,border:`1px solid ${ct.color}33`,padding:"6px 10px"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:ct.color}}></div><span style={{fontSize:13,color:ct.color,fontWeight:600}}>{ct.name}</span><span style={{fontSize:10,color:COLORS.textMuted}}>小タグ</span></div><div style={{display:"flex",gap:6}}><Btn onClick={()=>startEdit(ct)} style={{padding:"2px 8px",fontSize:11}}>編集</Btn><Btn variant="danger" onClick={()=>archive(ct.id)} style={{padding:"2px 8px",fontSize:11}}>アーカイブ</Btn></div></div><EditRow t={ct}/></div>))}</div>}
          </div>
        ))}
      </div>
      {archivedTags.length>0&&<div style={{marginTop:24}}><button onClick={()=>setShowArchived(!showArchived)} style={{background:"none",border:"none",color:COLORS.textMuted,fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6,marginBottom:12}}>{showArchived?"▼":"▶"} アーカイブ済み ({archivedTags.length})</button>{showArchived&&<div style={{display:"flex",flexDirection:"column",gap:8}}>{archivedTags.map(t=>(<div key={t.id} style={{background:COLORS.surface,borderRadius:10,padding:"10px 16px",border:`1px solid ${COLORS.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",opacity:.6}}><div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:10,height:10,borderRadius:"50%",background:t.color}}></div><span style={{fontSize:13,color:COLORS.textSoft}}>{t.name}</span><span style={{fontSize:10,color:COLORS.textMuted}}>{t.parentId?"小タグ":"親タグ"}</span></div><div style={{display:"flex",gap:6}}><Btn onClick={()=>restore(t.id)} style={{padding:"3px 10px",fontSize:11}}>復元</Btn><Btn variant="danger" onClick={()=>setTags(ts=>ts.filter(x=>x.id!==t.id))} style={{padding:"3px 10px",fontSize:11}}>完全削除</Btn></div></div>))}</div>}</div>}
    </div>
  );
};
...
export default function App(){
  ...
  const handleUpdateTask=updated=>{updT(updTree(tasks,updated.id,()=>updated));setDragTask(null);};
  ...
  return (
    <>
      ...
      <div style={{minHeight:"100vh",background:COLORS.bg,display:"flex"}}>
        <div style={{width:216,flexShrink:0,background:COLORS.surface,borderRight:`1px solid ${COLORS.border}`,display:"flex",flexDirection:"column",padding:"24px 0",position:"fixed",top:0,left:0,height:"100vh",overflowY:"auto",zIndex:10}}>
          ...
          <div style={{padding:"14px 10px",flex:1}}>{NAV.map(n=>(<button key={n.id} className="nb" onClick={()=>setView(n.id)} style={{display:"flex",alignItems:"center",gap:10,width:"100%",padding:"10px 12px",borderRadius:10,marginBottom:3,background:view===n.id?COLORS.accentSoft:"transparent",color:view===n.id?COLORS.accent:COLORS.textSoft,border:view===n.id?`1px solid ${COLORS.accent}33`:"1px solid transparent",fontSize:13,fontWeight:view===n.id?700:400,textAlign:"left"}}><span style={{fontSize:15}}>{n.icon}</span>{n.label}</button>))}</div>
          <div style={{padding:"14px 10px",borderTop:`1px solid ${COLORS.border}`}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}><Checkbox checked={filters.hideCompleted} onChange={()=>setFilters(f=>({...f,hideCompleted:!f.hideCompleted}))} size={16}/><span style={{fontSize:12,color:COLORS.textMuted}}>完了を隠す</span></div>
        ...
        <div style={{marginLeft:216,flex:1,display:"flex",minHeight:"100vh"}}>
          <div style={{flex:1,padding:32,minWidth:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28}}>
              <div><h1 style={{fontFamily:"'Space Grotesk',sans-serif",fontWeight:700,fontSize:22,letterSpacing:-.5}}>{NAV.find(n=>n.id===view)?.icon} {NAV.find(n=>n.id===view)?.label}</h1><div style={{fontSize:12,color:COLORS.textMuted,marginTop:2}}>{new Date(today).toLocaleDateString("ja-JP",{year:"numeric",month:"long",day:"numeric",weekday:"long"})}</div></div>
              {["list","day","week","month"].includes(view)&&<Btn variant="accent" onClick={()=>{setDefaultDate(null);setDefaultTime(null);setEditTask(null);setAddChildTo(null);setShowForm(true);}}>+ タスクを追加</Btn>}
            </div>
            ...
            {view==="day"&&<DayView tasks={tasks} tags={tags} today={today} onUpdateTask={handleUpdateTask} onAddTask={handleAddTask} onToggle={handleToggle} onEdit={handleEdit} onDelete={handleDelete} dragTask={dragTask}/>}
            {view==="week"&&<WeekView tasks={tasks} tags={tags} today={today} onUpdateTask={handleUpdateTask} onAddTask={handleAddTask} onToggle={handleToggle} onEdit={handleEdit} onDelete={handleDelete} dragTask={dragTask}/>}
            {view==="month"&&<MonthView tasks={tasks} tags={tags} today={today} onUpdateTask={handleUpdateTask} onAddTask={handleAddTask} onToggle={handleToggle} onEdit={handleEdit} onDelete={handleDelete} dragTask={dragTask}/>}
